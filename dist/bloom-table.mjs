...
function getCellVAlign(cell) {
	assert$2(cell.classList.contains("bloom-cell"), "getCellVAlign: not a cell");
	const v = cell.getAttribute("data-valign");
	return v === "top" || v === "center" || v === "bottom" ? v : null;
}
function setCellVAlign(cell, align) {
	assert$2(cell.classList.contains("bloom-cell"), "setCellVAlign: not a cell");
...
//#endregion
//#region src/resize-boundary-highlight.ts
const kThickness = 4;
let highlightDiv = null;
function ensureHighlightDiv() {
...
*/
function showResizeBoundaryHighlight(table, type, boundary) {
	...
	if (type === "row") Object.assign(div.style, {
...
function hideResizeBoundaryHighlight() {
...
function classifyCellSides(table, scopeCells, cell, grid = buildGrid(table)) {
	const outerEverywhere = {
		top: "outer",
		right: "outer",
		bottom: "outer",
		left: "outer"
	...
	const slotsAcross = {
		...
		bottom: Array.from({ length: sx }, (_, i) => [pos.row + sy, pos.column + i]),
		left: Array.from({ length: sy }, (_, i) => [pos.row + i, pos.column - 1]),
		right: Array.from({ length: sy }, (_, i) => [pos.row + i, pos.column + sx])
	};
...
*  tab in the menu. */
function scopeHasInnerSides(table, scopeCells) {
	const grid = buildGrid(table);
...
function applyVerticalAlignment(table, scope, cells, align) {
	withHistory(table, "Set Vertical Alignment", `${align}, ${describeTarget(table, scope, cells)}`, () => {
...
function applyBorderProps(table, scope, cells, props, edgeSet = "all") {
	const changed = describeBorderProps(props);
	const edges = edgeSet === "outer" ? ", outer edges" : edgeSet === "inner" ? ", inner edges" : "";
	if (scope === "table") {
		withHistory(table, "Change Border", (changed ? `${changed}, ${kWholeTableTarget}` : kWholeTableTarget) + edges, () => {
	...
	const sidesOf = edgeSet === "all" ? null : (() => {
		const grid = buildGrid(table);
		return cells.map((c) => classifyCellSides(table, cells, c, grid));
	})();
	if (sidesOf && !sidesOf.some((s) => kCellSides.some((side) => s[side] === edgeSet))) return;
	const target = describeTarget(table, scope, cells);
	withHistory(table, "Change Border", (changed ? `${changed}, ${target}` : target) + edges, () => {
...
function applyBorderColor(table, scope, cells, color, edgeSet = "all") {
...
function applyBorderToEdge(table, cell, side, brush) {
	withHistory(table, "Change Border", `${describeBorderProps(brush)}, ${side} edge of ${describeTarget(table, "cell", [cell])}`, () => {
...
function applyBorderStyle(table, scope, cells, style, edgeSet = "all") {
	applyBorderProps(table, scope, cells, { style }, edgeSet);
}
function applyBorderWeight(table, scope, cells, weight, edgeSet = "all") {
...
const kScopeAllIconSvg = `<svg width="20" height="20" viewBox="0 0 20 20" style="display:block"><rect x="3" y="3" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2"/><path d="M10 3v14M3 10h14" stroke="currentColor" stroke-width="2"/></svg>`;
const kScopeOuterIconSvg = `<svg width="20" height="20" viewBox="0 0 20 20" style="display:block"><path d="M10 3v14M3 10h14" stroke="#c9cdcf" stroke-width="1"/><rect x="3" y="3" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2"/></svg>`;
const kScopeInnerIconSvg = `<svg width="20" height="20" viewBox="0 0 20 20" style="display:block"><rect x="3" y="3" width="14" height="14" fill="none" stroke="#c9cdcf" stroke-width="1"/><path d="M10 3v14M3 10h14" stroke="currentColor" stroke-width="2"/></svg>`;
const kScopeBrushIconSvg = `<svg width="20" height="20" viewBox="0 0 20 20" style="display:block"><path d="M13.6 2.9l3.5 3.5-7.4 7.4-3.5-3.5z" fill="currentColor"/><path d="M5.5 11.2L2.6 17.4l6.2-2.9z" fill="currentColor"/></svg>`;
const kBorderBrushCursorUrl = `url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='28' height='28' viewBox='0 0 28 28'><path d='M19 2.5l6.5 6.5-11 11-6.5-6.5z' fill='%231d94a4' stroke='white' stroke-width='1.6'/><path d='M7.2 14.8L2.5 25.5l10.7-4.7z' fill='%2319818f' stroke='white' stroke-width='1.6'/></svg>") 3 25, crosshair`;
//#endregion
...
let borderBrushExiter = () => {};
function setBorderBrushExiter(fn) {
...
let brushMode = null;
function isBorderBrushModeActive() {
	return !!brushMode;
}
...
*  right track. */
function edgeAtPoint(cell, clientX, clientY, band = 6) {
	...
	for (const [side, d] of distances) if (d <= band && d < bestDistance) {
		best = side;
		bestDistance = d;
	}
	return best;
...
function ensureBorderBrushStyle() {
	if (document.head.querySelector(`style[data-table-overlay="${kBrushStyleTag}"]`)) return;
...
function clearPreview() {
	document.querySelectorAll(`[data-table-overlay="${kPreviewTag}"]`).forEach((el) => el.remove());
	previewFor = null;
}
...
function showPreview(cell, side) {
	if (!brushMode) return;
	clearPreview();
	previewFor = {
		cell,
...
function paintableCellAt(target) {
...
function onBrushPointerDown(e) {
	...
	if (!cell) {
		...
		const stays = !!target && (brushMode.owner.contains(target) || !!target.closest("[data-btable-menu], .bloom-table"));
...
function enterBorderBrushMode(owner, brush) {
	exitBorderBrushMode();
...
var DragToResize = class {
	...
	*/
	resizeEdgeAtPoint(event) {
		...
		if (resizeInfo) showResizeBoundaryHighlight(resizeInfo.table, resizeInfo.type, resizeInfo.boundary);
	...
	updateHighlightForDrag() {
		...
		try {
			...
			for (let i = 0; i < cells.length; i++) {
				...
				if ((type === "row" ? row + spanY - 1 : column + spanX - 1) !== index) continue;
...
function makeControlRow(label, controls, opts = {}) {
...
function makeIconToggle(icon, title, active, onClick, iconColor = kBloomBlue) {
...
function setColorInputValue(root, label, value) {
	const input = root.querySelector(`input[type="color"][aria-label="${label}"]`);
	if (!input) return;
	const isSet = /^#[0-9a-fA-F]{6}$/.test(value);
	input.value = isSet ? value : "#ffffff";
	const noColor = Array.from(input.parentElement?.children ?? []).find((el) => el !== input && el.tagName === "DIV");
	if (noColor) noColor.style.display = isSet ? "none" : "block";
}
function makeColorRow(entries, opts = {}) {
	const inset = opts.inset !== false;
	const wrap = document.createElement("div");
	wrap.style.padding = inset ? "4px 14px" : "4px 0";
	wrap.style.boxSizing = "border-box";
...
function makeScopeTabs(tabs, selectedId, onSelect) {
	...
	for (const tab of tabs) {
		...
		if (!tab.disabled) {
			...
			b.addEventListener("mouseleave", () => {
				if (b.getAttribute("aria-selected") !== "true") b.style.background = kScopeUnselectedFill;
			});
			b.addEventListener("click", (e) => {
				e.stopPropagation();
...
function makeScopeGroup(tabs, panel) {
...
const CommandRow = (props) => /* @__PURE__ */ jsxs(MenuItem, {
...
const ChoiceRow = (props) => /* @__PURE__ */ jsxs("li", {
	style: {
		display: "block",
		padding: "4px 14px",
		listStyle: "none"
	...
	children: [/* @__PURE__ */ jsx("div", {
		...
		children: props.item.options.map((option) => {
			...
			return /* @__PURE__ */ jsx(ToggleButton, {
				...
				children: option.icon ? /* @__PURE__ */ jsx(IconSlot, {
					icon: option.icon,
					color: kBloomBlue
				}) : /* @__PURE__ */ jsx("span", {
					style: {
...
const SectionHeading = (props) => /* @__PURE__ */ jsx("li", {
	style: {
		display: "block",
		listStyle: "none",
...
const FormatControls = (props) => {
	...
	return /* @__PURE__ */ jsxs("li", {
		...
		children: [/* @__PURE__ */ jsx("div", {
			...
			children: props.label
		}), /* @__PURE__ */ jsx("div", { ref: container })]
	});
};
const CellMenuItems = (props) => {
	...
	if (items.length > 0) rows.push(/* @__PURE__ */ jsx(SectionHeading, { label: localize(kCellSectionHeadingLabel, kCellSectionHeadingId) }, "heading"));
	for (const item of items) {
		if (groupOfPreviousItem && item.group !== groupOfPreviousItem) rows.push(/* @__PURE__ */ jsx(Divider, {}, `divider-before-${item.id}`));
		groupOfPreviousItem = item.group;
		if (item.kind === "choice") rows.push(/* @__PURE__ */ jsx(ChoiceRow, {
			item,
			localize,
			onChosen: () => noteCellChanged((count) => count + 1)
		}, item.id));
		else if (item.kind === "formatControls") {
			if (props.renderFormatControls) rows.push(/* @__PURE__ */ jsx(FormatControls, {
				label: localize(item.label, item.id),
				render: props.renderFormatControls
			}, item.id));
		} else rows.push(/* @__PURE__ */ jsx(CommandRow, {
			item,
			label: localize(item.label, item.id),
...
function mountCellMenuItems(container, props) {
	...
	flushSync(() => {
		root.render(/* @__PURE__ */ jsx(CellMenuItems, { ...props }));
	});
	return () => {
...
//#endregion
//#region src/components/icons/cell-merge-down.svg
var cell_merge_down_default = "data:image/svg+xml,<svg width=\"23\" height=\"23\" viewBox=\"0 0 23 23\" fill=\"none\" xmlns=\"http://www.w3.org/2000/svg\">%0A<g transform=\"rotate(90 11.5 11.5)\">%0A<path d=\"M9 6.53674V2.03674C9 1.7606 8.77615 1.53674 8.5 1.53674H1.5C1.22386 1.53674 1 1.7606 1 2.03674V21.0367C1 21.3129 1.22386 21.5367 1.5 21.5367H8.5C8.77615 21.5367 9 21.3129 9 21.0367V16.5367\" stroke=\"white\" stroke-width=\"2\" stroke-linecap=\"round\"/>%0A<path d=\"M13 16.5367V21.0367C13 21.3129 13.2239 21.5367 13.5 21.5367H20.5C20.7761 21.5367 21 21.3129 21 21.0367V2.03674C21 1.7606 20.7761 1.53674 20.5 1.53674H13.5C13.2239 1.53674 13 1.7606 13 2.03674V6.53674\" stroke=\"white\" stroke-width=\"2\" stroke-linecap=\"round\"/>%0A<path d=\"M1.5 11.5367H16.2544\" stroke=\"white\" stroke-width=\"2\" stroke-linecap=\"round\"/>%0A<path d=\"M15.6875 13.946L16.483 13.1505L18.074 11.5595L16.483 9.96847L15.6875 9.17297\" stroke=\"white\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"/>%0A</g>%0A</svg>%0A";
//#endregion
...
var align_left_default = "data:image/svg+xml,<svg width=\"40\" height=\"40\" viewBox=\"0 0 40 40\" fill=\"none\" xmlns=\"http://www.w3.org/2000/svg\">%0A<path d=\"M35 2H5C3.34315 2 2 3.34315 2 5V35C2 36.6569 3.34315 38 5 38H35C36.6569 38 38 36.6569 38 35V5C38 3.34315 36.6569 2 35 2Z\" stroke=\"black\" stroke-width=\"2\" stroke-linecap=\"round\"/>%0A<path d=\"M8 14H24M8 20H32M8 26H24\" stroke=\"black\" stroke-width=\"2\" stroke-linecap=\"round\"/>%0A</svg>%0A";
//#endregion
//#region src/components/icons/align-center.svg
var align_center_default = "data:image/svg+xml,<svg width=\"40\" height=\"40\" viewBox=\"40 0 40 40\" fill=\"none\" xmlns=\"http://www.w3.org/2000/svg\">%0A<path d=\"M75 2H45C43.3431 2 42 3.34315 42 5V35C42 36.6569 43.3431 38 45 38H75C76.6569 38 78 36.6569 78 35V5C78 3.34315 76.6569 2 75 2Z\" stroke=\"black\" stroke-width=\"2\" stroke-linecap=\"round\"/>%0A<path d=\"M52 14H68M48 20H72M52 26H68\" stroke=\"black\" stroke-width=\"2\" stroke-linecap=\"round\"/>%0A</svg>%0A";
//#endregion
//#region src/components/icons/align-right.svg
var align_right_default = "data:image/svg+xml,<svg width=\"40\" height=\"40\" viewBox=\"80 0 40 40\" fill=\"none\" xmlns=\"http://www.w3.org/2000/svg\">%0A<path d=\"M115 2H85C83.3431 2 82 3.34315 82 5V35C82 36.6569 83.3431 38 85 38H115C116.657 38 118 36.6569 118 35V5C118 3.34315 116.657 2 115 2Z\" stroke=\"black\" stroke-width=\"2\" stroke-linecap=\"round\"/>%0A<path d=\"M96 14H112M88 20H112M96 26H112\" stroke=\"black\" stroke-width=\"2\" stroke-linecap=\"round\"/>%0A</svg>%0A";
//#endregion
//#region src/components/icons/align-top.svg
var align_top_default = "data:image/svg+xml,<svg width=\"40\" height=\"40\" viewBox=\"120 0 40 40\" fill=\"none\" xmlns=\"http://www.w3.org/2000/svg\">%0A<path d=\"M155 2H125C123.343 2 122 3.34315 122 5V35C122 36.6569 123.343 38 125 38H155C156.657 38 158 36.6569 158 35V5C158 3.34315 156.657 2 155 2Z\" stroke=\"black\" stroke-width=\"2\" stroke-linecap=\"round\"/>%0A<path d=\"M128 8H152\" stroke=\"black\" stroke-width=\"2\" stroke-linecap=\"round\"/>%0A</svg>%0A";
//#endregion
//#region src/components/icons/align-middle.svg
var align_middle_default = "data:image/svg+xml,<svg width=\"40\" height=\"40\" viewBox=\"160 0 40 40\" fill=\"none\" xmlns=\"http://www.w3.org/2000/svg\">%0A<path d=\"M195 2H165C163.343 2 162 3.34315 162 5V35C162 36.6569 163.343 38 165 38H195C196.657 38 198 36.6569 198 35V5C198 3.34315 196.657 2 195 2Z\" stroke=\"black\" stroke-width=\"2\" stroke-linecap=\"round\"/>%0A<path d=\"M168 20H192\" stroke=\"black\" stroke-width=\"2\" stroke-linecap=\"round\"/>%0A</svg>%0A";
//#endregion
//#region src/components/icons/align-bottom.svg
var align_bottom_default = "data:image/svg+xml,<svg width=\"40\" height=\"40\" viewBox=\"200 0 40 40\" fill=\"none\" xmlns=\"http://www.w3.org/2000/svg\">%0A<path d=\"M235 2H205C203.343 2 202 3.34315 202 5V35C202 36.6569 203.343 38 205 38H235C236.657 38 238 36.6569 238 35V5C238 3.34315 236.657 2 235 2Z\" stroke=\"black\" stroke-width=\"2\" stroke-linecap=\"round\"/>%0A<path d=\"M208 32H232\" stroke=\"black\" stroke-width=\"2\" stroke-linecap=\"round\"/>%0A</svg>%0A";
//#endregion
//#region src/components/icons/menu-row.svg
var menu_row_default = "data:image/svg+xml,<svg width=\"28\" height=\"14\" viewBox=\"0 0 28 14\" fill=\"none\" xmlns=\"http://www.w3.org/2000/svg\">%0A<rect x=\"1.5\" y=\"1.5\" width=\"25\" height=\"11\" rx=\"1.5\" stroke=\"white\" stroke-width=\"2\"/>%0A</svg>%0A";
//#endregion
//#region src/components/icons/menu-column.svg
var menu_column_default = "data:image/svg+xml,<svg width=\"14\" height=\"28\" viewBox=\"0 0 14 28\" fill=\"none\" xmlns=\"http://www.w3.org/2000/svg\">%0A<rect x=\"1.5\" y=\"1.5\" width=\"11\" height=\"25\" rx=\"1.5\" stroke=\"white\" stroke-width=\"2\"/>%0A</svg>%0A";
//#endregion
...
let borderScopeMode = "all";
const kScopeCaptions = {
	all: {
...
function buildBorderScopeGroup(ctx, scope, table, cells, seed) {
	...
	let mode = borderScopeMode === "inner" && !hasInner ? "all" : borderScopeMode;
	const sidesBySet = (set) => {
		const list = cells();
		if (set === "all") return list.map((c) => [c, kCellSides]);
		const grid = buildGrid(table);
		return list.map((c) => {
			const cls = classifyCellSides(table, list, c, grid);
			return [c, kCellSides.filter((s) => cls[s] === set)];
		});
	};
	const agreedInSet = (set) => {
		let style;
		let weight;
		let seen = false;
		let color;
		...
		for (const [c, sides] of sidesBySet(set)) {
			...
			for (const s of sides) {
				...
				const hex = colors[s] ? toHexColor(colors[s]) : void 0;
				if (!hex) continue;
				if (color === void 0) color = hex;
	...
	const readSet = () => mode === "brush" ? "all" : mode;
	...
	const loadBrush = () => {
		...
		brush = {
			style: agreed.style === void 0 || agreed.style === "mixed" ? "solid" : agreed.style,
			weight: agreed.weight === void 0 || agreed.weight === "mixed" ? 1 : agreed.weight,
	...
	const refreshBorderToggles = () => {
		...
		styleButtons.forEach((b) => setToggleActive(b, shown.style !== void 0 && shown.style !== "mixed" && b.dataset.style === shown.style));
		weightButtons.forEach((b) => setToggleActive(b, shown.weight !== void 0 && shown.weight !== "mixed" && Number(b.dataset.weight) === shown.weight));
		colorValue = shown.color;
		if (colorRow) setColorInputValue(colorRow, "Border color", colorValue);
	};
	const colorRow = offersColor ? makeColorRow([{
		label: "Border color",
		value: colorValue,
		onInput: (color) => {
			colorValue = color;
	...
	for (const style of [
		...
		const b = makeBorderStyleToggle(style, () => {
			if (mode === "brush") {
				...
				updateBorderBrush(brush);
			} else applyBorderStyle(table, scope, cells(), style, mode);
			refreshBorderToggles();
		});
		styleButtons.push(b);
	}
	const styleRow = offersStyle ? makeControlRow("Border Style", styleButtons, { inset: false }) : null;
	...
	for (const weight of [
		...
		const b = makeBorderWeightToggle(weight, () => {
			if (mode === "brush") {
				...
				updateBorderBrush(brush);
			} else applyBorderWeight(table, scope, cells(), weight, mode);
			refreshBorderToggles();
		});
		weightButtons.push(b);
	}
	if (offersWeight) panel.appendChild(makeControlRow("Border Weight", weightButtons, { inset: false }));
	const group = makeScopeGroup(makeScopeTabs([
		{
			id: "all",
			label: "All",
			icon: kScopeAllIconSvg
		},
		{
			id: "outer",
			label: "Outer",
			icon: kScopeOuterIconSvg
		},
		{
			id: "inner",
			label: "Inner",
			icon: kScopeInnerIconSvg,
			disabled: !hasInner
		},
		{
			id: "brush",
			label: "Border Brush",
			icon: kScopeBrushIconSvg
		}
	], mode, (id) => {
		const chosen = id;
		if (chosen === "brush") loadBrush();
		else {
			exitBorderBrushMode();
...
function buildFormattingControls(ctx, scope) {
	...
	const vAligns = [
		{
			id: "top",
			icon: align_top_default,
			title: "Top"
		},
		{
			id: "center",
			icon: align_middle_default,
			title: "Middle"
		},
		{
			id: "bottom",
			icon: align_bottom_default,
			title: "Bottom"
		}
	];
	const vAlignButtons = [];
	...
	if (menuOffers(ctx, "fill")) els.push(makeColorRow([{
		label: "Fill",
		value: fillValue,
...
function onDocMouseDownForMenu(e) {
	const t = e.target;
	if (!t) return;
	if (isBorderBrushModeActive() && t instanceof Element && t.closest(".bloom-table")) return;
...
function menuMergeCellDown(cellToActOn) {
	const cell = cellToActOn ?? getMenuCell();
	const table = (cell ? ownerTable(cell) : null) ?? getMenuTable();
	if (!table || !cell) return;
	try {
...
*/
function canGrowSpan(cell, dx, dy) {
	const table = cell.closest(".bloom-table");
	if (!table) return false;
	try {
		...
		for (let r = row; r < row + newY; r++) for (let c = column; c < column + newX; c++) {
			if (r < row + spanY && c < column + spanX) continue;
	...
	return /* @__PURE__ */ jsxs("svg", {
		...
		children: [
			...
			/* @__PURE__ */ jsx("rect", {
				...
				children: /* @__PURE__ */ jsx("title", { children: "Toggle left border" })
			}),
			useBevel && /* @__PURE__ */ jsxs(Fragment, { children: [/* @__PURE__ */ jsx("rect", {
				x: leftRect.x,
				y: leftRect.y,
				width: 1,
				height: leftRect.h,
			...
			/* @__PURE__ */ jsx("rect", {
				...
				onClick: () => toggle("top"),
				style: { cursor: "pointer" },
				children: /* @__PURE__ */ jsx("title", { children: "Toggle top border" })
			}),
			useBevel && /* @__PURE__ */ jsxs(Fragment, { children: [/* @__PURE__ */ jsx("rect", {
				x: topRect.x,
				y: topRect.y,
			...
			/* @__PURE__ */ jsx("rect", {
				...
				style: { cursor: "pointer" },
				children: /* @__PURE__ */ jsx("title", { children: "Toggle right border" })
			}),
			useBevel && /* @__PURE__ */ jsxs(Fragment, { children: [/* @__PURE__ */ jsx("rect", {
				x: rightRect.x,
				y: rightRect.y,
			...
			/* @__PURE__ */ jsx("rect", {
				...
				style: { cursor: "pointer" },
				children: /* @__PURE__ */ jsx("title", { children: "Toggle bottom border" })
			}),
			useBevel && /* @__PURE__ */ jsxs(Fragment, { children: [/* @__PURE__ */ jsx("rect", {
				x: bottomRect.x,
				y: bottomRect.y,
			...
			showInner && /* @__PURE__ */ jsxs("g", {
				opacity: selected.has("innerH") || selected.has("innerV") ? 1 : .5,
				onClick: () => toggle("inner"),
				style: { cursor: "pointer" },
				children: [
					/* @__PURE__ */ jsx("title", { children: "Toggle inner borders" }),
					/* @__PURE__ */ jsx("rect", {
						x: innerHRect.x,
						y: innerHRect.y,
						width: innerHRect.w,
...
const BorderMenu = (props) => {
	...
	return /* @__PURE__ */ jsxs("div", {
		...
		children: [/* @__PURE__ */ jsx("button", {
			...
			children: options.map((opt) => /* @__PURE__ */ jsxs("div", {
				role: "menuitemradio",
				...
				children: [opt.icon ? opt.icon() : null, !hideLabels && /* @__PURE__ */ jsx("span", {
					style: { color: "#1f3a40" },
					children: opt.label
				})]
			}, String(opt.value)))
...
function WeightSampleLine({ value, style, color = "#fff", fullWidth }) {
	if (value === "mixed") return /* @__PURE__ */ jsx("span", {
		style: { fontSize: 11 },
		children: "Mixed"
	});
	if (value === 0) {
		...
		return fullWidth ? /* @__PURE__ */ jsx("div", {
			style: {
				width: "100%",
				display: "flex",
				justifyContent: "center"
	...
	return /* @__PURE__ */ jsx("svg", {
		...
		"aria-hidden": true,
		children: visible ? style === "double" ? /* @__PURE__ */ jsxs(Fragment, { children: [/* @__PURE__ */ jsx("line", {
			x1: xStart,
			y1: "7",
			x2: xEnd,
			y2: "7",
...
function WeightMenu(props) {
	return /* @__PURE__ */ jsx(BorderMenu, {
		label: "Weight",
		value: props.value,
		options: options$2.map((o) => ({
			...
			icon: () => /* @__PURE__ */ jsx("div", {
				style: { width: "100%" },
				children: /* @__PURE__ */ jsx(WeightSampleLine, {
					value: o.value,
		...
		renderButtonImage: (v) => /* @__PURE__ */ jsx(WeightSampleLine, {
			value: v,
			style: props.currentStyle
		}),
		hideLabels: true
...
function StyleSampleLine({ value, color = "#fff", fullWidth }) {
	...
	if (value === "double") return /* @__PURE__ */ jsxs("svg", {
		width: svgWidth,
		height: 18,
		viewBox: `0 0 ${viewBoxWidth} 18`,
		"aria-hidden": true,
...
function StyleMenu(props) {
	return /* @__PURE__ */ jsx(BorderMenu, {
		...
		options: options$1.map((o) => ({
			...
			icon: () => /* @__PURE__ */ jsx("div", {
				style: { width: "100%" },
				children: /* @__PURE__ */ jsx(StyleSampleLine, {
					value: o.value,
					color: "#1f3a40",
					fullWidth: true
				})
		...
		onChange: props.onChange,
		disabled: props.disabled,
		renderButtonImage: (v) => /* @__PURE__ */ jsx(StyleSampleLine, { value: v }),
		hideLabels: true
	});
}
//#endregion
...
function BorderControl(props) {
	...
	const [valueMap, setValueMap] = useState(props.valueMap);
	const [selected, setSelected] = useState(() => props.initialSelected ?? computeInitialSelection(props.valueMap, showInner));
	...
	const weight = useMemo(() => computeMixedWeight(valueMap, selected), [valueMap, selected]);
	const style = useMemo(() => computeMixedStyle(valueMap, selected), [valueMap, selected]);
	...
	return /* @__PURE__ */ jsx("div", { children: /* @__PURE__ */ jsxs("div", {
		...
		children: [/* @__PURE__ */ jsx(BorderSelector, {
			valueMap,
			showInner,
			selected,
...
function CornerMenu(props) {
	return /* @__PURE__ */ jsx(BorderMenu, {
		...
		options: options.map((o) => ({
			value: o.value,
			label: o.label,
			icon: () => /* @__PURE__ */ jsx(CornerSampleImage, {
				value: o.value,
				color: "#1f3a40"
			})
		})),
		onChange: props.onChange,
		disabled: props.disabled,
		renderButtonImage: (v) => /* @__PURE__ */ jsx(CornerSampleImage, { value: v }),
		hideLabels: true
	});
}
//#endregion
...
const TableApiContext = createContext(defaultTableApi);
/** Read the injected table api (falls back to defaultTableApi via the context). */
const useTableApi = () => useContext(TableApiContext);
//#endregion
...
const ColorPickerContext = createContext(DefaultColorPicker);
...
const TableSection = ({ table }) => {
	...
	const [cornerValue, setCornerValue] = useState(domCornerValue);
	const lastEmitted = useRef(domCornerValue);
	useEffect(() => {
	...
	return /* @__PURE__ */ jsx(Section, {
		...
		children: table && /* @__PURE__ */ jsx(Fragment, { children: (() => {
			...
			return /* @__PURE__ */ jsxs(Fragment, { children: [
				/* @__PURE__ */ jsx("div", {
					...
					children: /* @__PURE__ */ jsx(BorderControl, {
				...
				/* @__PURE__ */ jsx("div", {
					className: menuItemStyle$1,
					style: { cursor: "default" },
					children: /* @__PURE__ */ jsx(CornerMenu, {
						value: cornerValue,
						onChange: (v) => {
							if (!table) return;
...
const IconButton = ({ icon, alt, onClick, title, className, style, iconSize = 24, children, selected, sx, ...rest }) => /* @__PURE__ */ jsx(MuiIconButton, {
	onClick,
	onMouseDown: (e) => e.preventDefault(),
	className,
	"aria-label": alt,
...
const RadioGroup = ({ options, value, onChange, className, disabled, label, ...rest }) => {
	...
	return /* @__PURE__ */ jsx("div", {
		...
		children: options.map((opt, idx) => {
			...
			return /* @__PURE__ */ jsxs(React.Fragment, { children: [/* @__PURE__ */ jsx(IconButton, {
				icon: opt.icon,
				...
				children: !opt.icon && /* @__PURE__ */ jsx("div", {
					style: {
						whiteSpace: "pre-wrap",
						textAlign: "center",
						lineHeight: 1.1,
	...
	return /* @__PURE__ */ jsxs(Section, {
		...
		children: [
			/* @__PURE__ */ jsxs("div", {
				...
				}), currentCell && currentType && /* @__PURE__ */ jsx(RadioGroup, {
					className: "ml-2",
					label: "Content type",
					disabled,
					value: currentType,
			...
			/* @__PURE__ */ jsxs("div", {
				...
				}), currentCell && /* @__PURE__ */ jsx(RadioGroup, {
					className: "ml-2",
			...
			/* @__PURE__ */ jsxs("div", {
				...
				children: [/* @__PURE__ */ jsx("div", {
					className: "text-sm opacity-80 mb-2",
					children: "Corners"
				}), currentCell && /* @__PURE__ */ jsx(CornerMenu, {
					disabled,
			...
			/* @__PURE__ */ jsxs("div", {
				...
				children: [/* @__PURE__ */ jsx("div", {
					className: "text-sm opacity-80 mb-2",
					children: "Padding"
				}), currentCell && /* @__PURE__ */ jsx(Slider, {
					className: "ml-2",
...
const TableMenu = (props) => {
	...
	const lastCellLocation = useRef(null);
	useEffect(() => {
	...
	const inertWrapRef = useRef(null);
	useEffect(() => {
	...
	return /* @__PURE__ */ jsx(TableApiContext.Provider, {
		value: api,
		children: /* @__PURE__ */ jsx(ColorPickerContext.Provider, {
			value: ColorPicker,
			children: /* @__PURE__ */ jsxs("div", {
				ref: panelRef,
				...
				children: [
					...
					/* @__PURE__ */ jsxs("div", {
						...
						children: [
							/* @__PURE__ */ jsx(TableSection, { table }),
							/* @__PURE__ */ jsx(RowSection, {
								table,
								currentCell,
								disabled: !hasContext,
